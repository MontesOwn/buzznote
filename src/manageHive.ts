...
async function submitData(formData: FormData, hiveId: number, boxId: number | null) {
    try {
        ...
        if (!boxId) {
            ...
            newBox['box_id'] = parseInt(response['box_id']);
            ...
            if (response['box_id']) {
                ...
                if (boxesTable) {
                    ...
                    rows.forEach(row => {
                        if (row.id === response['box_id'].toString()) {
                            const newRow = createRowForListTable(response, columnHeaders, response['box_id'].toString());
                            const editButton = newRow.querySelector('button') as HTMLElement;
                            editButton.addEventListener('click', () => openModalForBox(hiveId, newBox['box_id']));
                            tbody.replaceChild(newRow, row);
                            createMessage("Box updated successfully", "main-message", "check_circle");
                        }
                    })
                }
            } else {
                createMessage("Please reload the page", "main-message", "info");
            }
        }
        closeModal('manage-hive-backdrop');
    } catch (error: any) {
