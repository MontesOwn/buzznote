...
async function searchHives() {
    try {
        ...
        const buttonGroup = hives.reduce((acc: HTMLElement, currentHive: Hive) => {
            ...
            const hiveButton = createButton(hiveButtonText, "button", currentHive['hive_id'].toString(), "button white");
            hiveButton.addEventListener('click', async () => {
                const filteredHives = await getInspectionsForHiveID(currentHive['hive_id']);
